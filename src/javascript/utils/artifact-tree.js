        ...
        me._copyStandaloneArtifacts(overrides).then({
            success: function(){
                ...
                Deft.Chain.sequence([
                    ...
                    success: function(){
                        this.logger.log('set parent', parent && parent.get('_ref') || "No Parent");
                        var root = me.tree[me.rootArtifact.get('ObjectID')].copyRecord,
                            parentRef = parent && parent.get('_ref') || null;

                        if (parent && parentRef){
                            parent.get('_type') == this.portfolioItemTypes[0].toLowerCase() ? root.set("PortfolioItem", parentRef):root.set("Parent", parentRef);
                        }
